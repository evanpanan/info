...
import type { ChannelItem, ChannelItemSection } from '../types/irpr';

type InternalChannelItem = ChannelItem & {
  Icon?: LucideIcon;
...
const DEFAULT_INTERNAL: InternalChannelItem[] = [
  { ...DEFAULT_CHANNELS[0], Icon: Home },
  { ...DEFAULT_CHANNELS[1], Icon: FileBarChart },
  { ...DEFAULT_CHANNELS[2], Icon: Bot },
  { ...DEFAULT_CHANNELS[3], Icon: Sparkles },
  { ...DEFAULT_CHANNELS[4], CustomLogo: XLogo },
  { ...DEFAULT_CHANNELS[5], CustomLogo: RedditLogo },
  { ...DEFAULT_CHANNELS[6], Icon: Youtube },
  { ...DEFAULT_CHANNELS[7], Icon: Linkedin },
  { ...DEFAULT_CHANNELS[8], CustomLogo: FutuLogo },
  { ...DEFAULT_CHANNELS[9], CustomLogo: LaohuLogo },
  { ...DEFAULT_CHANNELS[10], CustomLogo: HSTLogo },
  { ...DEFAULT_CHANNELS[11], CustomLogo: XBelieversLogo },
  { ...DEFAULT_CHANNELS[12], CustomLogo: NotionLogo },
];

export type EditChannelDraft = Partial<
  Pick<ChannelItem, 'name' | 'url' | 'description' | 'badge' | 'section' | 'iconBg' | 'iconSrc'>
> & {
  IconName?: string;
};
...
function toInternal(item: ChannelItem, idx: number): InternalChannelItem {
  const base = DEFAULT_INTERNAL[idx];
  if (base && base.id === item.id) {
    return { ...base, ...item };
  }
  return { ...item } as InternalChannelItem;
}

export default function OfficialMatrix({
  initialItems,
  isIRPRAdmin = false,
}: {
  ...
  const [items, setItems] = useState<InternalChannelItem[]>(
    initialItems ? initialItems.map(toInternal) : DEFAULT_INTERNAL
  );
  ...
  const [draft, setDraft] = useState<EditChannelDraft>({ name: '', url: '', description: '', section: 'official' });
  ...
  const openAdd = (section: ChannelItemSection) => {
  ...
  const commitDraft = () => {
    ...
    if (!cleanName) return;
    if (editor.mode === 'add') {
      const next: InternalChannelItem = {
        id: `link-${Date.now()}`,
        name: cleanName,
        url: (draft.url ?? '').trim(),
          ...
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, section: e.target.value as ChannelItemSection }))
                    }
            ...
            <div className="px-5 py-3.5 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between">
              <div className="text-[11.5px] text-slate-400">
                {isIRPRAdmin ? '管理员修改将立即生效' : '仅管理员可修改'}
              </div>
