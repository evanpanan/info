...
export interface FeedPublisherProps {
  isIRPRAdmin?: boolean;
  canPublish?: boolean;
...
export default function FeedPublisher({
  isIRPRAdmin = false,
  canPublish,
  currentAvatar,
  onPublish,
  ...
  const [aiBanner, setAIBanner] = useState<{
    visible: boolean;
    source: 'images' | 'files' | 'both';
    msg?: string;
  }>({ visible: false, source: 'both' });
  ...
  const setContentSanitized = (
    next:
      | string
      | number
      | null
      | undefined
      | ((prev: string) => string | number | null | undefined)
  ) => {
    let value = '';
  ...
  const clearDetectedLink = () => {
    ...
    const removed = sanitizeInput(content.replace(detectedUrl, ''))
      .replace(/\s{3,}/g, '  ')
      .trim();
  ...
  const runAISummary = async (next: { images: ImageAttach[]; files: FileAttach[] }) => {
    ...
    try {
      ...
      setAIBanner({
        visible: true,
        source,
        msg: typeof onAISummarize === 'function' ? 'AI 整理完成' : '未配置 LLM，已展示结构化占位摘要',
      });
    } finally {
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // #region debug-point C:file-selected
    reportDebug('C', 'FeedPublisher.tsx:handleFileSelected', 'publisher created object urls', {
      count: attached.length,
      files: attached.map((f) => ({ id: f.id, name: f.name, type: f.type, objectUrl: f.objectUrl })),
    });
    // #endregion
  ...
  const handlePublish = () => {
    const { strippedContent } = extractAndStripUrls(content, detectedPreview ?? null);
    const hasImages = attachedImages.length > 0;
    const hasFiles = attachedFiles.length > 0;
    if (!strippedContent && !detectedPreview && !hasImages && !hasFiles) return;
    ...
    const finalFiles: FileAttachment[] | undefined = hasFiles
      ...
      : undefined;
    // #region debug-point D:publish-files
    reportDebug('D', 'FeedPublisher.tsx:handlePublish', 'publisher is emitting files', {
      hasFiles,
      finalFiles: finalFiles?.map((f) => ({ id: f.id, name: f.name, url: f.url, type: f.type })) ?? [],
    });
    // #endregion
    ...
    setAttachedImages(() => []);
    setAttachedFiles((prev) => {
      // #region debug-point E:revoke-after-publish
      reportDebug('E', 'FeedPublisher.tsx:handlePublish:revoke', 'publisher is retaining published object urls', {
        count: prev.length,
        files: prev.map((f) => ({ id: f.id, name: f.name, objectUrl: f.objectUrl })),
      });
      // #endregion
      return [];
    });
  ...
  if (!(canPublish || isIRPRAdmin)) return null;

  const avatarSrc = currentAvatar ?? 'https://i.pravatar.cc/80?img=32';
        ...
        <div className="flex-1 min-w-0">
          ...
          {videoEmbed && (
            <div className="mt-3 rounded-2xl border border-slate-200 overflow-hidden bg-slate-50">
              <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-slate-200 bg-white">
                <div className="flex items-center gap-2 text-xs text-slate-700 min-w-0">
                  ...
                  <span className="truncate">
                    {videoEmbed.provider === 'youtube' ? `YouTube · ${videoEmbed.videoId ?? videoEmbed.url}` : videoEmbed.url}
                  </span>
                </div>
                <button
                  type="button"
              ...
              <div className="relative w-full aspect-video bg-black flex items-center justify-center text-white text-[11px] sm:text-xs">
                {videoEmbed.provider === 'youtube' && videoEmbed.videoId ? (
                  ...
                  <a href={videoEmbed.url} target="_blank" rel="noreferrer noopener" className="underline">
                    点击打开视频：{videoEmbed.url}
                  </a>
                )}
              </div>
            </div>
          ...
          {attachedFiles.length > 0 && (
            <div className="mt-3 space-y-2">
              {attachedFiles.map((f) => (
                <div
                  key={f.id}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-slate-200"
                >
